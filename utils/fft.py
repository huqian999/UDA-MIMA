...
def FDA_source_to_target(src_img, trg_img, L=0.1):
    # get fft of both source and target傅里叶变换
    fft_src = torch.fft.rfft2(src_img.clone(), dim=(-2, -1))
    fft_trg = torch.fft.rfft2(trg_img.clone(), dim=(-2, -1))

    # extract amplitude and phase of both ffts振幅和相位
    amp_src, pha_src = extract_ampl_phase(fft_src.clone())
    amp_trg, pha_trg = extract_ampl_phase(fft_trg.clone())

    # replace the low frequency amplitude part of source with that from target将源的低频振幅部分替换为目标的低频振幅
